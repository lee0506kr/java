...
표현식은 특정 테이블에 데이터가 있는것이 아니기 때문에 임의의 데이터에서 
    가져 올 수 있음 dual
ex) select empno,ename,'입니다.' from emp;
  
2. 칼럼명을 바꿔서 조회할 수 있다. (Alias)
  select [칼럼명] as [별명], 
         [칼럼명] as [별명]
    from [테이블명]
 ...
 key :  하나의 로우(레코드)를 구분할 수 있는 데이터
     ...
     후보키 : 키 중에, 최소 단위 그룹
     복합키 : 여러개의 칼럼을 한꺼번에 비교 했을 때 특정 레코드를 조회할 수 
            있으면 복합키
    
     외래키 : 다른테이블에서 기본키로 사용하고 있는 데이터 (foreign key)
------------------------------------------------------
...
**급여가 4000이상인 직원의 이름과 업무와 급여를 조회
  ...
  조건식을 여러개 쓰고 싶을 때는  AND,OR 를 이용   
   **부서 번호가 30인 직원 중, 
   급여가 1500 이상인 직원의 이름, 업무, 급여,부서번호를 조회  
 ...
 like : 문자열 비교, 문자열 포함 여부를 검사할 때 사용 	   
   ...
   ** 업무가 SALESMAN 또는 MANGER 인 직원의 이름과 업무 조회
     ...
     ** MGR(매니저)이 null 인 직원의 모든정보 조회
    select * 
      from emp
     where mgr is null;
    
     ** comm이 null이 아닌 직원의 이름, 직책, 급여 를 조회
  ...
  between A and B : 특정 범위를 조건으로 줄때 사용
   ...
   -----------------------------------    
    ...
    order by : 조회 정렬 기준설정
    모든직원의 모든 정보를 조회, 급여 기준 오름차순 정렬
    (내림차순 정렬은 desc)
    select * 
      from emp
     order by sal desc;
 --------------------------------------------------      
  1. 부서 번호가 10이고 업무가 CLERK 인 직원의 이름, 업무, 부서 번호 조회
    select ename,job,deptno from emp 
     where deptno = 10 and job = 'CLERK';
  2. 입사일이 81년인 모든 직원의 모든정보 조회
    select * from emp where hiredate 
     between '81/01/01' and '81/12/31'
  3. 매니저의 직원번호가 7698 인 직원의 이름,부서, 업무 조회
     select ename,deptno,job from emp where mgr = 7698
  4. 업무가 SALESMAN인 직원 중, 급여가 1500 이상인 직원의 모든정보 조회
  5. (부서번호가 10인 직원 중, 업무가 CLERK 인 직원 )+ 
      업무가 SALESMAN인 직원의 이름, 부서번호,업무, 급여, 커미션을 조회 
     select ename,deptno,job,sal,comm
       from emp
      where (deptno = 10 and job like 'CLERK')
         or (job like 'SALESMAN');
  ...
  7. 업무가 SALESMAN인 직원의 이름,급여, 커미션 출력
      단, 커미션이 500이상인 경우만 출력, 커미션 기준 내림차순 정렬
    
        select ename, sal, comm
          where job like 'SALESMAN' and comm >=500
         order by comm desc;
