...
LENGTH : 문자열 길이 반환
	emp테이블에서 이름의 길이가 5이상인 모든 직원의 이름을 조회
...
--------------------------------------------------
날짜 관련 함수
날짜 출력 포맷, 날짜계산, 일수확인 등등..	

SYSDATE : 시스템 현재 날짜 반환
	select sysdate from dual;
TO_DATE :특정 포맷의 문자열을 날짜 데이터로 변환
	select to_date('2018-05-09','RRRR-MM-dd') from dual;
...
ADD_MONTHS : 특장날짜에 개월 수 더한 날짜 반환
...
ROUND : 낮 12시(정오) 기준으로 시간이 지났으면 다음날, 안지났으면 오늘 반환
	...
	select round(
	   to_date('2018-05-09 13:00','RRRR-MM-dd hh24:mi')) 
	from dual;
TRUNC : 해당 날짜 반환(00시 기준)
	select to_char(trunc(sysdate),'RRRR-MM-dd hh24:mi:ss') from dual;

--------------------------------------------------
NVL(칼럼,값) : 조회한 칼럼이 null 이면 지정한 값을 넣어줌 
	emp 테이블에서 모든 직원의 이름과, sal + comm 더한 급여를 조회
...
DECODE(A,B,값)	
	...
	select deptno,decode(deptno,10,'ACCOUNTING',20,'RESEARCH','기타부서') from emp;
